...
if (!function_exists('add_css'))
{
	function add_css($css = NULL)
	{
		...
		if (!is_array($css))
		{
			...
			$url = (!preg_match('#^www|^http|^//#', $css)) ? base_url('assets/'.$css.$file_type) : $css;
		...
		else
		{
			...
			foreach ($css as $item)
			{
				...
				$url = (!preg_match('#^www|^http|^//#', $item)) ? base_url('assets/'.$item.$file_type) : $item;
...
if (!function_exists('add_js'))
{
	function add_js($js = NULL)
	{
		...
		if (!is_array($js))
		{
			...
			$url = (!preg_match('#^www|^http|^//#', $js)) ? base_url('assets/'.$js.$file_type) : $js;
